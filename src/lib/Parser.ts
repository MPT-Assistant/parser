...
class Parser {
    ...
    const parseTeacher = (element: Element): void => {
      ...
      if (!isTeacherName(teacherName)) {
        ...
        while (currentElement && !isTeacherName(teacherName)) {
          teacherName = this._normalizeTeacherName(
        ...
        if (!isTeacherName(teacherName)) {
          ...
          const parentText = this._normalizeTeacherName(
            parent.text().trim()
          ).split("");
      ...
      teachers.push({
        ...
        photo: element.attribs.src.startsWith("/")
          ? `${this._reaHost}${element.attribs.src}`
          : element.attribs.src,
  ...
  private _parseTeacherName(
    teacherName: string
  ): Pick<ITeacher, "name" | "surname" | "patronymic"> {
  ...
  private async _loadMptPage(
    path: (typeof mptPages)[keyof typeof mptPages] | string
  ): Promise<CheerioAPI> {
    const html = (
  ...
  private async _loadReaPage(
    path: (typeof reaPages)[keyof typeof reaPages]
  ): Promise<CheerioAPI> {
    const html = (
